...
#include <stdio.h>      // Standard C header file from CdePkg\Include\VS2022 
#include <string.h>     // Standard C header file from CdePkg\Include\VS2022 
...
EFI_STATUS  main(int argc, char** argv)
{
  ...
  // CdePkg demo
  //
  EFI_HANDLE          ImageHandle = (void*)argv[-2];    // get ImageHandle from CdePkg backdoor
  IN EFI_SYSTEM_TABLE* SystemTable = (void*)argv[-1];   // get SystemTable from CdePkg backdoor
  if (1) {
