...
{
  return AeadAesGcmEncrypt(key, key_size, iv, iv_size,
                           a_data, a_data_size, data_in, data_in_size,
                           tag_out, tag_size, data_out, data_out_size);
}
...
bool libspdm_aead_aes_gcm_decrypt(const uint8_t *key, size_t key_size,
                                  ...
                                  uint8_t *data_out, size_t *data_out_size)
{
  return AeadAesGcmDecrypt(key, key_size, iv, iv_size,
                           a_data, a_data_size, data_in, data_in_size,
...
bool libspdm_rsa_pkcs1_sign_with_nid(void *rsa_context, size_t hash_nid,
                                     ...
                                     size_t *sig_size)
{
  return RsaPkcs1SignWithNid(rsa_context, hash_nid, message_hash, hash_size,
                             signature, sig_size);
}
...
bool libspdm_rsa_pkcs1_verify_with_nid(void *rsa_context, size_t hash_nid,
                                       ...
                                       size_t sig_size)
{
  return RsaPkcs1VerifyWithNid(rsa_context, hash_nid, message_hash, hash_size,
                               signature, sig_size);
}
...
{
  return X509GetExtensionData(cert, cert_size, oid, oid_size,
                              extension_data, extension_data_size);
}
...
bool libspdm_x509_get_cert_from_cert_chain(const uint8_t *cert_chain,
                                           ...
                                           size_t *cert_length)
{
  return X509GetCertFromCertChain(cert_chain, cert_chain_length,
                                  cert_index, cert, cert_length);
}
...
{
  return X509GetExtendedBasicConstraints(cert, cert_size,
                                         basic_constraints, basic_constraints_size);
}
...
{
  return EcDsaSign(ec_context, hash_nid, message_hash,
...
{
  return EcDsaVerify(ec_context, hash_nid, message_hash,
                     hash_size, signature, sig_size);
}
...
{
  return HkdfSha256ExtractAndExpand(key, key_size, salt, salt_size,
...
{
  return HkdfSha256Extract(key, key_size, salt, salt_size,
...
bool libspdm_hkdf_sha256_expand(const uint8_t *prk, size_t prk_size,
                                ...
                                uint8_t *out, size_t out_size)
{
  return HkdfSha256Expand(prk, prk_size, info, info_size,
                          out, out_size);
}
...
bool libspdm_hkdf_sha384_extract_and_expand(const uint8_t *key, size_t key_size,
                                            ...
                                            uint8_t *out, size_t out_size)
{
  return HkdfSha384ExtractAndExpand(key, key_size, salt, salt_size,
                                    info, info_size, out, out_size);
}
...
bool libspdm_hkdf_sha384_extract(const uint8_t *key, size_t key_size,
                                 ...
                                 uint8_t *prk_out, size_t prk_out_size)
{
  return HkdfSha384Extract(key, key_size, salt, salt_size,
                           prk_out, prk_out_size);
}
...
bool libspdm_hkdf_sha384_expand(const uint8_t *prk, size_t prk_size,
                                ...
                                uint8_t *out, size_t out_size)
{
  return HkdfSha384Expand(prk, prk_size, info, info_size,
                          out, out_size);
}
