        ...
        for VarInfoFile in VarInfoFilelist:
            ...
            for vardata in Data:
                if vardata['VendorGuid'] == 'NA':
                    continue
                if (vardata['VendorGuid'], vardata["VarName"]) in VarDataDict and vardata in VarDataDict[(vardata['VendorGuid'], vardata["VarName"])]:
                    continue
            ...
            for storeid in range(storenum):
                for Var in item.fields[storeid]:
                    ...
                    if Var.Offset not in Name_Offset:
                        VariableInfo.append('  defaultstore %-5s | %-30s | %-15s | %-15s | %s'%(storeid, Var.Offset, Var.Type, Var.Offset, Var.Value))
