...
Demonstration on how to transform existing Standard C software project to run in the UEFI Shell
...
This blog will demonstrate how surprisingly easy the port to the UEFI Shell could be done
if a *Standard C API* is available.[<sup>2</sup>](footnotes/footnote-2.md)

The tools provided by the ACPICA are really useful during the development process of PC-platforms.
The extraction, comparison and disassembly of binary ACPI tables could be done easily directly 
on the target.
...
A datatype ```long``` of 32Bit-size in x86-64 mode had to be fixed necessarily.

## Modified project sourcecode
Only the Microsoft-specific project files have been  transfered and modified to the 
...
Additionally a couple of Windows functions needs to be rewritten for UEFI usage.
...
This functions have been provided by the [Win324UEFI.lib](https://github.com/KilianKegel/Win324UEFI).
...
and ```AcpiReleaseGlobalLock()``` needs to be rewritten from inline assembly language to
...
#### acwin.h
For the same regards the macros in ```acwin.h```have to be adjusted.
...
The original **toro-C-Library** source code is included in that project.

This is for informational purpose only.

Currently **toro-C-Library** has this functions implemented:<br>
...
1. install FLEX and BISON that is required for the *AslCompiler*:<br>
...
The .EFI files appeares in the ```x64\UEFIShell``` folder.
