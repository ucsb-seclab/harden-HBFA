  ...
  )
{
  ...
  mVariableFv = AllocatePool ((UINTN)((EFI_FIRMWARE_VOLUME_HEADER *)tc3_var_fv)->FvLength);
  if (mVariableFv == NULL) {
    return UNIT_TEST_ERROR_PREREQUISITE_NOT_MET;
  }

  CopyMem (mVariableFv, tc3_var_fv, sizeof (tc3_var_fv));
...
UNIT_TEST_STATUS
EFIAPI
TC3_ProtectedVariableLibInitialize (
  ...
  )
{
  ...
  UT_ASSERT_EQUAL (Status, EFI_SUCCESS);
  UT_ASSERT_EQUAL (Global->TableCount, 4);              // 4 valid variables in total
  ...
  UT_ASSERT_NOT_EQUAL (Global->UnprotectedVariables[2], 0);   // has VarErrLog variable
