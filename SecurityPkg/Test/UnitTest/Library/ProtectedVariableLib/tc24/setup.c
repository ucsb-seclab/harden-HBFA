  ...
  )
{
  ...
  mVariableFv = AllocatePool ((UINTN)((EFI_FIRMWARE_VOLUME_HEADER *)tc24_var_fv)->FvLength);
  if (mVariableFv == NULL) {
    return UNIT_TEST_ERROR_PREREQUISITE_NOT_MET;
  }
  CopyMem (mVariableFv, tc24_var_fv, sizeof (tc24_var_fv));
  ...
  )
{
  ...
  *Name = (CHAR16 *)Buffer;
  do {
    NameLength = tc24_data_pool[PoolIndex] & 0xF;
    PoolIndex = (PoolIndex + 1) % sizeof (tc24_data_pool);
  } while (NameLength < 2);

  PoolIndex = (PoolIndex + Scramble) % sizeof (tc24_data_pool);
  for (Index = 0; Index < NameLength; ++Index) {
    PoolIndex = (PoolIndex + Index) % sizeof (tc24_data_pool);
    NameChar = tc24_data_pool[PoolIndex] & 0x7F;
  ...
  *Data = Buffer;
  do {
    *DataSize = tc24_data_pool[PoolIndex];
    PoolIndex = (PoolIndex + 1) % sizeof (tc24_data_pool);
  } while (*DataSize == 0);
  ...
  for (Index = 0; Index < *DataSize; ++Index) {
    PoolIndex = (PoolIndex + Index) % sizeof (tc24_data_pool);
    *Buffer = tc24_data_pool[PoolIndex];
    Buffer += 1;
  }
