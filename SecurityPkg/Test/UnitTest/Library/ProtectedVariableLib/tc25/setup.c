  ...
  )
{
  ...
  mVariableFv = AllocatePool ((UINTN)((EFI_FIRMWARE_VOLUME_HEADER *)tc25_var_fv)->FvLength);
  if (mVariableFv == NULL) {
    return UNIT_TEST_ERROR_PREREQUISITE_NOT_MET;
  }
  CopyMem (mVariableFv, tc25_var_fv, sizeof (tc25_var_fv));
  ...
  )
{
  ...
  *Name = (CHAR16 *)Buffer;
  do {
    NameLength = tc25_data_pool[PoolIndex] & 0xF;
    PoolIndex = (PoolIndex + 1) % sizeof (tc25_data_pool);
  } while (NameLength < 2);

  PoolIndex = (PoolIndex + Scramble) % sizeof (tc25_data_pool);
  for (Index = 0; Index < NameLength; ++Index) {
    PoolIndex = (PoolIndex + Index) % sizeof (tc25_data_pool);
    NameChar = tc25_data_pool[PoolIndex] & 0x7F;
  ...
  *Data = Buffer;
  do {
    *DataSize = tc25_data_pool[PoolIndex];
    PoolIndex = (PoolIndex + 1) % sizeof (tc25_data_pool);
  } while (*DataSize == 0);
  ...
  for (Index = 0; Index < *DataSize; ++Index) {
    PoolIndex = (PoolIndex + Index) % sizeof (tc25_data_pool);
    *Buffer = tc25_data_pool[PoolIndex];
    Buffer += 1;
  }
