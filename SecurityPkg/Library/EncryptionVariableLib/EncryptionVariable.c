  ...
  )
{
  ...
  ASSERT (DataSize > sizeof (VARIABLE_ENCRYPTION_HEADER));
  ASSERT (EncHeader->DataType == ENC_TYPE_AES || EncHeader->DataType == ENC_TYPE_NULL);
  ASSERT (EncHeader->HeaderSize >= sizeof (VARIABLE_ENCRYPTION_HEADER));
  ASSERT (EncHeader->CipherDataSize > 0);
  ASSERT ((EncHeader->CipherDataSize % ENC_BLOCK_SIZE) == 0);
  ASSERT (EncHeader->PlainDataSize > 0);
  ASSERT (EncHeader->PlainDataSize <= EncHeader->CipherDataSize);
  ASSERT ((EncHeader->CipherDataSize + EncHeader->HeaderSize) <= DataSize);
  ...
  )
{
  ...
  if ((UINTN)CipherData == (UINTN)PlainData) {
    ...
    //
    CipherBuffer = (UINT8 *)AllocatePool (CipherDataSize
                                          + sizeof (VARIABLE_ENCRYPTION_HEADER));
  ...
  if (PaddingBytes != 0) {
    ...
    //
    PlainBuffer = AllocatePool (PlainDataSize + PaddingBytes);
    if (PlainBuffer == NULL) {
  ...
  )
{
  ...
  if (CipherData->DataType == ENC_TYPE_AES) {
    ...
    //
    PlainBuffer = AllocatePool (CipherDataSize);
    if (PlainBuffer == NULL) {
      ASSERT (PlainBuffer != NULL);
      return EFI_OUT_OF_RESOURCES;
    }
  ...
  if (!EFI_ERROR (Status)) {
    if (PlainBuffer != NULL && PlainBuffer != PlainData) {
