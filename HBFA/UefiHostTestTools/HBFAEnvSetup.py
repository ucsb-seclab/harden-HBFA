...
def HelpInfo():
    ...
    print("  --reconfig            Overwrite the UefiHostFuzzTestPkg/Conf/*.txt")
    print("                        files with the template files from the BaseTools/Conf directory.\n")
    print("Please note: This script must be put under HBFA/UefiHostTestTools, so the Conf can be generated.")


if len(sys.argv) == 2:
    if sys.argv[1] == '-h' or sys.argv[1] == '-?' or sys.argv[1] == '--help' or sys.argv[1] == 'help':
        HelpInfo()
        os._exit(0)
...
if 'EDK_TOOLS_PATH' in os.environ:
    ...
    print ("EDK_TOOLS_PATH should be set, or you need to run edksetup.sh/ edksetup.bat")
    os._exit(0)
...
ExtendDefineFile = os.path.join(HBFA_PATH, 'UefiHostFuzzTestPkg', 'Conf', 'tools_def.customized')
...
def ExtendOptionToBuildRules(Src, Dst, SectionName, TargetOption, ExtendOption):
    src = open(Src, 'r')
    lines = src.readlines()
    src.close()
...
'''
        ...
        ExtendOptionToBuildRules(os.path.join(ConfPath, 'build_rule.txt'), os.path.join(ConfPath, 'build_rule.txt'),
                                 SectionNames, TargetOption, ExtendOption)
    if (not os.path.exists(os.path.join(ConfPath, 'tools_def.txt'))) or ReConfig:
        GenerateNewToolsDef(os.path.join(ConfSrc, 'tools_def.template'), os.path.join(ConfPath, 'tools_def.txt'))


def main(ReConfig):
    GenerateConfCustomized(ConfTemplate, os.path.join(HBFA_PATH, 'UefiHostFuzzTestPkg', 'Conf'), ReConfig)
