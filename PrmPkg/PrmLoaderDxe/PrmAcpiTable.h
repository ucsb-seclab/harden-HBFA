...
typedef struct {
  ...
  GUID                                HandlerGuid;                ///< GUID of the PRM handler for this structure
  UINT32                              Reserved;                   ///< Reserved for future use
  UINT64                              HandlerPhysicalAddress;     ///< Physical address of this PRM handler
...
typedef struct {
  ...
  GUID                                ModuleGuid;                 ///< GUID of the PRM module for this structure
  UINT16                              ModuleMajorRevision;        ///< PRM module major revision
  UINT16                              ModuleMinorRevision;        ///< PRM module minor revision
  UINT32                              NumberPrmHandlerInfo;       ///< Number of entries in the Handler Info array
  UINT32                              OffsetPrmHandlerInfo;       ///< Offset in bytes from the beginning of this
  ...
  PRM_HANDLER_INFORMATION_STRUCT      PrmHandlerInfoStructure[1];
} PRM_MODULE_INFORMATION_STRUCT;

typedef struct {
  EFI_ACPI_DESCRIPTION_HEADER         Header;                     ///< Standard ACPI description header
  UINT32                              OffsetPrmModuleInfo;        ///< Offset in bytes from the beginning of this
                                                                  ///< structure to the PRM Module Info array
  UINT32                              NumberPrmModuleInfo;        ///< Number of entries in the PRM Module Info array
...
//
#define PRM_MODULE_INFORMATION_STRUCTURE(ModuleGuid, ModuleRevision, NumberPrmHandlerInfo, PrmHanderInfoStructureArray) {               \
    {                                                                                                                                        \
      PRM_MODULE_INFORMATION_STRUCT_REVISION,                              /* UINT16    StructureRevision;   */                         \
      (OFFSET_OF (PRM_MODULE_INFORMATION_STRUCT, PrmHandlerInfoStructure) + (NumberPrmHandlerInfo * sizeof (PRM_HANDLER_INFORMATION_STRUCT)))   /* UINT16    StructureLength;     */ \
      ModuleGuid,                                                          /* GUID      ModuleGuid;          */                         \
      ModuleRevision,                                                      /* UINT16    ModuleRevision       */                         \
      NumberPrmHandlerInfo,                                                /* UINT16    NumberPrmHandlerInfo */                         \
      OFFSET_OF (PRM_MODULE_INFORMATION_STRUCT, OffsetPrmHandlerInfo),     /* UINT32    OffsetPrmHandlerInfo */                         \
      PrmHanderInfoStructureArray                                          /* PRM_HANDLER_INFORMATION_STRUCT PrmHandlerInfoStructure */ \
    } \
  }

#define PRM_HANDLER_INFORMATION_STRUCTURE(HandlerGuid, HandlerPhysicalAddress) {                                                   \
    {                                                                                                                              \
      ...
      HandlerPhysicalAddress,                                              /* UINT64                  HandlerPhysicalAddress  */   \
    } \
  }
